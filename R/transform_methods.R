...
transform_pload.list_linescore <- function(payload_obj, ...) {
    ...
    payload_obj$game %<>% dplyr::mutate(away_team_link = if (exists('away_team_link', where = payload_obj$game)) away_team_link else NA,
                                        highlights_available = if (exists('highlights_available', where = payload_obj$game)) highlights_available else NA,
                                        home_team_link = if (exists('home_team_link', where = payload_obj$game)) home_team_link else NA,
                                        pbp_last = if (exists('pbp_last', where = payload_obj$game)) pbp_last else NA,
                                        preview = if (exists('preview', where = payload_obj$game)) preview else NA,
