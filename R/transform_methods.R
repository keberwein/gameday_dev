...
transform_pload.list_bis_boxscore <- function(payload_obj, ...) {
    ...
    payload_obj$batting %<>%
        ...
        .[,`:=` (id=as.numeric(id), bo=as.numeric(bo), ab=as.numeric(ab), po=as.numeric(po),
    ...
    payload_obj$pitching %<>%
        data.table::data.table() %>%
        .[, `:=` (id=as.numeric(id), out=as.numeric(out), bf=as.numeric(bf), er=as.numeric(er),
...
transform_pload.df_game_events <- function(payload_obj, ...) {
    ...
    payload_obj %<>%
        data.table::data.table() %>%
        .[, `:=` (start_speed=as.double(start_speed), num=as.numeric(num),
...
transform_pload.df_inning_hit <- function(payload_obj, ...) {
    ...
    payload_obj %<>%
        data.table::data.table() %>%
        .[, `:=` (x=as.double(x), y=as.double(y), batter=as.numeric(batter), pitcher=as.numeric(pitcher),
                  inning=as.numeric(inning))]
...
transform_pload.list_inning_all <- function(payload_obj, ...) {
    payload_obj$atbat %<>%
        ...
        .[, play_guid := if(exists('play_guid', where = payload_obj$atbat)) play_guid else NA] %>%
        ...
        .[, c("pitcher", "batter", "num", "b", "s", "o", "start_tfs", "start_tfs_zulu", "stand", "b_height", 
              "p_throws", "atbat_des", "atbat_des_es", "event", "score", "home_team_runs", "away_team_runs", 
              "url", "inning_side", "inning", "next_", "event2", "event3", "batter_name", "pitcher_name", 
              "event4", "gameday_link", "date", "end_tfs_zulu", "event_num", "event_es", "play_guid", "event2_es"), with = FALSE]
        
    
    payload_obj$action %<>%
        data.table::data.table() %>%
        # Add columns that may not exist.
        .[, play_guid := if(exists('play_guid', where = payload_obj$action)) play_guid else NA] %>%
        ...
        data.table::setcolorder(c("b", "s", "o", "des", "des_es", "event", "tfs", "tfs_zulu", "player", "pitch", "url", 
                               "inning_side", "inning", "next_", "num", "score", "home_team_runs", "away_team_runs", 
                               "event2", "gameday_link", "event_es", "event_num", "play_guid", "event2_es"))
    ...
    payload_obj$pitch %<>%
        ...
        .[, `:=` (id = as.numeric(id), x = as.numeric(x), y = as.numeric(y), end_speed = as.numeric(end_speed),
                  sz_top = as.numeric(sz_top), sz_bot = as.numeric(sz_bot), pfx_x = as.numeric(pfx_x), pfx_z = as.numeric(pfx_z),
                  px = as.numeric(px), pz = as.numeric(pz), x0 = as.numeric(x0), y0 = as.numeric(y0), z0 = as.numeric(z0),
                  vx0 = as.numeric(vx0), vy0 = as.numeric(vy0), vz0 = as.numeric(vz0), ax = as.numeric(ax), ay = as.numeric(ay),
        ...
        .[, c("des", "des_es", "id", "type", "tfs", "tfs_zulu", "x", "y", "sv_id", "start_speed", "end_speed", "sz_top", 
              "sz_bot", "pfx_x", "pfx_z", "px", "pz", "x0", "y0", "z0", "vx0", "vy0", "vz0", "ax", "ay", "az", "break_y", 
              "break_angle", "break_length", "pitch_type", "type_confidence", "zone", "nasty", "spin_dir", "spin_rate", "cc", 
              "mt", "url", "inning_side", "inning", "next_", "num", "on_1b", "on_2b", "on_3b", "count", "gameday_link", "code", 
              "event_num", "play_guid"), with=FALSE]
        
    payload_obj$runner %<>%
        data.table::data.table() %>%
        .[, `:=` (id = as.numeric(id), num = as.numeric(num))] %>%
        .[, c("id", "start", "end", "event", "score", "rbi", "earned", "url", "inning_side", "inning", "next_", "num",
              "gameday_link", "event_num"), with=FALSE]
        
    
    payload_obj$po %<>% 
        data.table::data.table() %>%
        .[, play_guid := if (exists('play_guid', where = payload_obj$po)) play_guid else NA] %>%
        .[, play_guid := if (exists('catcher', where = payload_obj$po)) catcher else NA] %>%
        .[, play_guid := if (exists('des_es', where = payload_obj$po)) des_es else NA] %>%
        .[, play_guid := if (exists('event_num', where = payload_obj$po)) event_num else NA] %>%
        .[, c("des", "url", "inning_side", "inning", "next_", "num", "gameday_link", "des_es", 
              "event_num", "play_guid", "catcher"), with=FALSE]
...
transform_pload.list_linescore <- function(payload_obj, ...) {
    ...
    payload_obj$game %<>% 
        ...
        .[, c("ampm", "aw_lg_ampm", "away_ampm", "away_code", "away_division", "away_file_code",       
              ...
              "venue_w_chan_loc", "wrapup_link", "xmlns.xs", "date", "runner_on_2b", "runner_on_3b", "runner_on_1b"), with=FALSE]
...
transform_pload.df_game <- function(payload_obj, ...) {
    ...
    payload_obj %<>% 
        data.table::data.table() %>%
        .[, `:=` (id=as.numeric(id), w=as.numeric(w), l=as.numeric(l), league_id=as.numeric(league_id),
