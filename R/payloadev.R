...
payloadev <- function(start=NULL, end=NULL, league="mlb", source="statcast", dataset = NULL, game_ids = NULL, db_con = NULL, overwrite = FALSE, ...){
    ...
    ifelse(source==tolower("pitchrx"), args <- structure(args, class="pitchfx"), args <- structure(args, class="statcast"))
    
}


#' Method for payloadev objects.
...
payload.pitchfx <- function(args, ...) {
    ...
    # Make urlz a subclass of pitchfx.
    ...
    if(!is.null(args$db_con)){
        ...
        for(i in seq_along(url_chunks)){
            ...
            if(args$dataset == "inning_all" | args$dataset=="linescore"){
                ...
                ######## Setting dataset to a class can get rid of all this, and maybe some stuff above.
                ######## Should prolly be a subclass of pitchfx.
                ######## why are these all inning hit?
                ######## maybe urlz shoul be the main subclass.
            } else {
                if(args$dataset=="inning_hit"){
                    innings_df <- payload.gd_inning_hit(urlz)
                    if(isTRUE(args$overwrite)) DBI::dbWriteTable(conn = args$db_con, value = innings_df, name = "inning_hit", overwrite = TRUE)
                    if(isTRUE(args$overwrite)) DBI::dbWriteTable(conn = args$db_con, value = innings_df, name = "inning_hit", append = TRUE)
                }
                if(args$dataset=="game_events"){
                    innings_df <- payload.gd_inning_hit(urlz)
                    if(isTRUE(args$overwrite)) DBI::dbWriteTable(conn = args$db_con, value = innings_df, name = "game_events", overwrite = TRUE)
                    if(isTRUE(args$overwrite)) DBI::dbWriteTable(conn = args$db_con, value = innings_df, name = "game_events", append = TRUE)                    
                }
                if(args$dataset=="game"){
                    innings_df <- payload.gd_inning_hit(urlz)
                    if(isTRUE(args$overwrite)) DBI::dbWriteTable(conn = args$db_con, value = innings_df, name = "game", overwrite = TRUE)
                    if(isTRUE(args$overwrite)) DBI::dbWriteTable(conn = args$db_con, value = innings_df, name = "game", append = TRUE)                         
                }
                if(args$dataset=="bis_boxscore"){
                    innings_df <- payload.gd_inning_hit(urlz)
                    if(isTRUE(args$overwrite)) DBI::dbWriteTable(conn = args$db_con, value = innings_df, name = "bis_boxscore", overwrite = TRUE)
                    if(isTRUE(args$overwrite)) DBI::dbWriteTable(conn = args$db_con, value = innings_df, name = "bis_boxscore", append = TRUE)  
                } 
            }
